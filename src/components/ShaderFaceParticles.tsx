...
export function ShaderFaceParticles() {
  ...
  const enhancedGeometry = useMemo(() => {
    ...
    for (let i = 0; i < positions.length; i += 3) {
      enhancedPositions.push(positions[i], positions[i + 1], positions[i + 2])
      const uvIndex = (i / 3) * 2
    ...
    for (let i = 0; i < positions.length - 9; i += 9) {
      ...
      // Sample 15 points per triangle for MASSIVE density
      for (let j = 0; j < 15; j++) {
        ...
        if (uv3Index + 1 < uvs.length) {
          ...
          enhancedUVs.push(0.5, 0.5) // Fallback UV
        }
      }
      ...
      for (let i = 0; i < positions.length - 6; i += 6) {
        ...
        for (let k = 0; k < 5; k++) {
          const t = (k + 1) / 6 // Interpolation factor
          ...
          if (uv2Index + 1 < uvs.length) {
            ...
            const v = (1 - t) * uvs[uv1Index + 1] + t * uvs[uv2Index + 1]
            enhancedUVs.push(u, v)
          } else {
            enhancedUVs.push(0.5, 0.5)
          }
        }
      }
    }
    ...
    newGeometry.setAttribute('position', new BufferAttribute(new Float32Array(enhancedPositions), 3))
    newGeometry.setAttribute('uv', new BufferAttribute(new Float32Array(enhancedUVs), 2))
    
    console.log(`Enhanced geometry: ${enhancedPositions.length / 3} particles`)
  ...
  return (
    <points ref={pointsRef} geometry={enhancedGeometry} material={shaderMaterial} />
  )
}